...
function signUp () {
	...
	if (foundUser) {
		document.getElementById('notallowed').innerHTML = 'Value exist'
	} else {
...
function transaction (amount, receiver) {
	var date = new Date()
	var transactionUser = users.find(function (user) {
	...
	if (receiver === loggedInUsername) {
		...
		if (transactionUser && loggedInUsername) {
			if (sum() >= amount) {
				userAmount.push({
		...
		} else if (transactionUser === undefined || transactionUser) {
			if (loggedInUsername === undefined) {
				return {
					success: false,
...
function sum () {
	var balanceUser = userAmount.filter(function (user) {
		return loggedInUsername === user.username
	})
	return (total = balanceUser.reduce(
...
function deposit () {
	...
	var date = new Date()
	var depositor = users.find(function (user) {
		return username === user.username
	})
	if (depositor) {
		userAmount.push({
